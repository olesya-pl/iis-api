...
namespace IIS.Core.GraphQL
{
    public class DummyMutation
    {
        ...
        public string FillDummyTypes([Service] OntologyContext context)
        {
            ...
            var nameattr = CreateAttribute(context, "Name", "Название",
                Core.Ontology.EntityFramework.Context.ScalarType.String);
            ...
            var ageattr = CreateAttribute(context, "Age", "Возраст",
                Core.Ontology.EntityFramework.Context.ScalarType.Int);
            CreateRelation(context, person, ageattr, RelationKind.Embedding);
            var homeattr = CreateAttribute(context, "HomeLocation", "Домашний адрес",
                Core.Ontology.EntityFramework.Context.ScalarType.Geo);
            CreateRelation(context, person, homeattr, RelationKind.Embedding);
            var photoattr = CreateAttribute(context, "Photo", "Фотография",
                Core.Ontology.EntityFramework.Context.ScalarType.File);
            CreateRelation(context, person, photoattr, RelationKind.Embedding);
            var birthattr = CreateAttribute(context, "BirthDate", "Дата рождения",
                Core.Ontology.EntityFramework.Context.ScalarType.Date);
        ...
        private Type CreateAttribute(OntologyContext context, string name, string title,
            Core.Ontology.EntityFramework.Context.ScalarType scalarType)
        {
