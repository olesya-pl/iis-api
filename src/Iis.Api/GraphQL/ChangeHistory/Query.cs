...
namespace IIS.Core.GraphQL.ChangeHistory
{
    public class Query
    {
            ...
            string propertyName = "")
        {
            ...
            var graphQLItems = items.Select(item => mapper.Map<ChangeHistoryItem>(item)).ToList();
            return new GraphQLCollection<ChangeHistoryItem>(graphQLItems, graphQLItems.Count);
        }
    }
}
