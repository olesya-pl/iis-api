...
namespace Iis.UnitTests.Iis.Elastic.Tests
{
    public class ElasticLoggerTests
    {
        ...
        {
            //arrange
            var sut = new ElasticLogUtils();

            //act
            var log = sut.PrepareLog(new StringResponse("{\"error\":{\"root_cause\":[{\"type\":\"index_not_found_exception\",\"reason\":\"no such index [ont_materials2]\",\"resource.type\":\"index_or_alias\",\"resource.id\":\"ont_materials2\",\"index_uuid\":\"_na_\",\"index\":\"ont_materials2\"}],\"type\":\"index_not_found_exception\",\"reason\":\"no such index [ont_materials2]\",\"resource.type\":\"index_or_alias\",\"resource.id\":\"ont_materials2\",\"index_uuid\":\"_na_\",\"index\":\"ont_materials2\"},\"status\":404}")
            {
            ...
            //assert
            Assert.Equal("Request GET with path https://kek.com/ completed. Success:False ", log.Message);
        ...
        public void Success_IndexCreated()
        {
            ...
            //act
            var log = sut.PrepareLog(new StringResponse("{\"acknowledged\":true,\"shards_acknowledged\":true,\"index\":\"ont_materials2\"}")
            {
                ApiCall = new ApiCallDetails
                {
                    HttpStatusCode = 200,
            ...
            //assert
            Assert.Equal("Request GET with path https://kek.com/ completed. Success:True ", log.Message);
        ...
        public void Success_QuerySuccess()
        {
            ...
            //assert
            Assert.Equal("Request GET with path https://kek.com/ completed. Success:True Took:48. Timed out:False.", log.Message);
            Assert.Equal(LogLevel.Information, log.LogLevel);
        }
    }
}
