...
namespace Iis.OntologyManager
{
    public partial class MainForm : Form
    {
        ...
        private void SetControlsTopPanel()
        {
            ...
            menuElastic.Items[1].Click += async (sender, e) => { await ReIndexElasticAsync(IndexKeys.Signs); };
            menuElastic.Items.Add("Індекси Подій");
            menuElastic.Items[2].Click += async (sender, e) => { await ReIndexElasticAsync(IndexKeys.Events); };
            menuElastic.Items.Add("Індекси Звітів");
            menuElastic.Items[3].Click += async (sender, e) => { await ReIndexElasticAsync(IndexKeys.Reports); };
            menuElastic.Items.Add("Індекси Матеріалів");
            menuElastic.Items[4].Click += async (sender, e) => { await ReIndexElasticAsync(IndexKeys.Materials); };
